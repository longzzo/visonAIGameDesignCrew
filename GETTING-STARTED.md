...
> **Vision Engine이 뭔가요?** — 게임 아이디어 한 줄을 넣으면, AI 에이전트 19명이 픽셀아트 사무실에서 일하는 1인 게임 개발 스튜디오입니다. **기획팀 12명**(PM·시나리오·게임플레이·시스템·UI/UX·밸런스·BM·아트·선임 개발자·스케줄러·마케팅·QA)이 회의해서 게임 기획서(GDD)를 쓰고 서로 검토·채점한 뒤, **개발팀 7명**(Unity 아키텍트·게임플레이 스크립터·멀티플레이·테크니컬 아티스트·에디터 툴·코드 리뷰어·테스트 엔지니어)이 이어받아 실제 개발을 돕습니다. 마지막엔 **유니티 개발 문서 + 밸런스 CSV + 에셋 목록 + C# 스켈레톤 + 플레이 가능한 프로토타입**을 ZIP 하나로 뽑아줍니다.
>
...
🩺 **에이전트 전원 점검**(사이드바)으로 19명 생존 확인. 특정 모델이 과부하일 수 있으니 다른 모델로 바꿔보세요.
...
├── config/openclaw.json        ← 에이전트 19명 설정 템플릿 (키 없음)
├── scripts/
