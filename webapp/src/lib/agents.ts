...
export function notionEditPrompt(
  ...
  pageMd: string,
  request: string,
  task: "revise" | "add" | "subpage" = "revise"
): string {
  if (task === "subpage") {
    // 하위 페이지 모드 — 부모 페이지 아래 들어갈 새 기획서 문서를 통째로 만든다
    return [
      `너는 노션 아키비스트다. 오너의 노션 페이지 「${title}」 아래에 들어갈 새 하위 기획서를 작성한다.`,
      ``,
      `[부모 페이지 원문 — 참고용, 다시 출력하지 마라]`,
      ...
      ``,
      `규칙:`,
      `- 첫 줄은 반드시 "# <이모지 1개> <문서 제목>" — 예: # ⚙️ 시장 시스템 (Market System)`,
      `- 본문 리듬: "## 1. 소제목" 숫자 헤딩 → **볼드 리드 한 문장** → 불릿 상세 → 필요한 곳에 헤더 행 포함 마크다운 표.`,
      `- 개조식(명사형 종결). 합니다체 금지. 요구보다 과하게 확장하지 마라.`,
      `- 부모 원문 재출력 금지, "[[유지: …]]" 마커 금지, 코드펜스 금지, 도구 호출 금지.`,
...
/** 발행 전 다듬기 — 아키비스트가 GDD 섹션을 오너 노션 스타일로 재포맷 (내용 불변) */
export function notionPolishPrompt(sectionTitle: string, body: string): string {
  return [
    `너는 노션 아키비스트다. 아래 기획 섹션을 오너의 노션 스타일로 재포맷해라 — 내용(사실·수치·결정)은 절대 바꾸지 말고 형태만 다듬는다.`,
    ``,
    `스타일 규칙:`,
    `- "### 1. 소제목" 숫자 소제목 → 바로 아래 **볼드 리드 한 문장** → 불릿 상세 순서.`,
    `- 표 데이터는 헤더 행 포함 마크다운 표로. 이미 표면 유지.`,
    `- 개조식(명사형 종결). 합니다체 금지. 원문에 없는 내용 추가 금지, 있는 내용 삭제 금지.`,
    ``,
    `[섹션: ${sectionTitle}]`,
    body.slice(0, 6000),
    ``,
    `출력: 재포맷된 본문만. 섹션 제목(## …) 재출력 금지, 사족·코드펜스·도구 호출 금지.`,
  ].join("\n");
}
